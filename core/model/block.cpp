...
// External CUDA Kernel Binding (defined in attention.cu)
...
torch::Tensor apply_rotary_emb(torch::Tensor x, int64_t /*dim*/) {
    ...
    auto inv_freq = 1.0 / torch::pow(
        10000.0,
        torch::arange(0, head_dim, 2, x.options()) / head_dim
    );
    auto t     = torch::arange(T, x.options());
    auto freqs = torch::einsum("i,j->ij", {t, inv_freq});

    auto freqs_cos = freqs.cos().repeat({1, 2}); // [T, head_dim]
    auto freqs_sin = freqs.sin().repeat({1, 2}); // [T, head_dim]

    // Rotate half
    auto x_rot = torch::cat(
        {-x.slice(3, head_dim / 2), x.slice(3, 0, head_dim / 2)}, 3
    );

    return x * freqs_cos.unsqueeze(0).unsqueeze(0) +
           x_rot * freqs_sin.unsqueeze(0).unsqueeze(0);
}
...
struct TransformerBlockMoE : torch::nn::Module {
    ...
    TransformerBlockMoE(const TitanConfig& cfg) {
        ...
        ln1 = register_module("ln1",
            torch::nn::LayerNorm(torch::nn::LayerNormOptions({cfg.n_embd}).eps(1e-5)));
    ...
    torch::Tensor forward(torch::Tensor x,
        ...
        auto topk_result    = torch::topk(router_probs, static_cast<int>(top_k), -1);
        auto routing_weights   = std::get<0>(topk_result);
        auto selected_experts  = std::get<1>(topk_result);

        routing_weights = routing_weights / routing_weights.sum(-1, true);

        auto final_output = torch::zeros_like(x);

        for (int i = 0; i < n_experts; ++i) {
            auto mask = (selected_experts == i).any(-1); // [B, T]
            if (mask.any().item<bool>()) {
                auto expert_out    = experts[i]->forward(x);
                auto weight_mask   = (selected_experts == i).to(x.dtype());
                auto weight_for_e  = (routing_weights * weight_mask).sum(-1, true); // [B, T, 1]
                final_output += expert_out * weight_for_e * mask.unsqueeze(-1).to(x.dtype());
            }
        }
